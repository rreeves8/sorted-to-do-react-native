...
export default function SortableTasks({ navigation }: any) {
    ...
    const [pickedData, setPickedData] = useState<PickerItem>({ label: "Benefits Quantity", value: 3 });
        ...
        >
            ...
            >
                {(() => {
                    ...
                    return (allTasks.length === 0) ? <Text style={{
                        alignSelf: 'center'
                    }}>No Tasks, add a task using the cateroies section</Text> : (
                        <SortedTask
                            taskList={allTasks}
                            nav={navigation}
                            sortType={pickedData?.value}
                        />
                    )
                })()}
                <View
